...
class ReadOnlyGraph<N : Enum<N>, R : Enum<R>>(private val nodeTypeEnum: Class<N>, private val relationTypeEnum: Class<R>, private val input: InputStream) : GraphOrdinalContainer<N>(true) {
    ...
    class OrdinalIterable(private val iterator: OrdinalIterator) : Iterable<Int> {
        override fun iterator(): Iterator<Int> {
            return OrdinalIteration(iterator)
        }
    }

    class OrdinalIteration(private val iterator: OrdinalIterator) : AbstractIterator<Int>() {
    ...
    fun NodeAndOrd<N>.getSingleConnection(relation: R): NodeAndId<N>? {
        return graph.getConnection(this, relation)
    }

    fun NodeAndOrd<N>.getSingleConnection(model: String, relation: R): NodeAndId<N>? {
        return graph.getConnection(model, this, relation)
    }

    fun NodeAndOrd<N>.getConnections(relation: R): Set<NodeAndId<N>> {
        return graph.getConnectionSet(this, relation)
    }

    fun NodeAndOrd<N>.getConnections(model: String, relation: R): Set<NodeAndId<N>>  {
        return graph.getConnectionSet(model, this, relation)
    }

    fun NodeAndId<N>.getSingleConnection(relation: R): NodeAndId<N>? {
    ...
    fun NodeAndId<N>.getConnections(relation: R): Set<NodeAndId<N>> {
