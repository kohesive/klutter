...
class ConfiguredValue(val cfg: Config, val key: String) {
    ...
    fun asPathList(): List<Path> = cfg.getStringList(key).map { Paths.get(it).toAbsolutePath() }
    fun asPathList(defaultValue: List<Path>): List<Path> = if (exists()) asPathList() else defaultValue
    fun asPathListOrNull(): List<Path>? = if (exists()) asPathList() else null
    fun asPathListOrEmpty(): List<Path> = asPathList(emptyList())
}
